...
- [G...XLIQ — Liquidator Deep Dive](XLIQ.md) ← 19 fills, $12.5M seized, Spoofing-tagged funding chain
- [G...YHNF — Liquidator Deep Dive](YHNF.md) ← 31 fills, protocol-connected, BLND whale ties
- [G...SXI3 — SDF Pool Position Deep Dive](SXI3.md) ← SDF-funded, 70.7M XLM liquidated, 11-signer multisig
...
[`G...SXI3`](SXI3.md) alone accounted for **93.6%** of all XLM seized across all 60 auctions.

**[`G...SXI3`](SXI3.md) split by filler:**
- [`G...XLIQ`](XLIQ.md) — 7 fills, **76,341,176.58 XLM** (98.1%)
- [`G...YHNF`](YHNF.md) — 2 fills, **1,507,194.95 XLM** (1.9%)
...
**Combined: ~70.68M XLM collateral** from liquidating position owned by [`GCA34H...SXI3`](SXI3.md) (SDF Pool Position).
...
**[`GAPU4...XLIQ`](XLIQ.md) was Fill #1** — first to react, 11 minutes after the exploit.

### Liquidator Accounts

| Account | Fills | Key Role | Deep Dive |
|:--------|:-----:|:---------|:---------:|
| `GAPU4WSJMFW6Q4G5CLADCNWFIAOGTCC5XXY3DVUA23W7ZPUIYSSGXLIQ` | 19 | First mover; took 70.7M XLM from GCA34H position | [→ XLIQ.md](XLIQ.md) |
| `GAIN2HU2ITUZPLGNWSH7JXXROL7MYIWDYXCUQDMQVJXFBEBPETSZYHNF` | 31 | Most fills overall; bulk of smaller positions | [→ YHNF.md](YHNF.md) |
...
## G...SXI3 — SDF Pool Position

> **📄 Full deep dive: [`SXI3.md`](SXI3.md)** — SDF funding network, 511.8M XLM wire history, inflow attribution, shared signer proof, BLND issuer verification

The largest liquidated position — `GCA34HBKNLWN3AOXWBRW5Y3HSGHCWF3UDBRJ5YHGU6HWGJZEPO2NSXI3` — was **not a retail user**. It is an 11-signer institutional multisig operated by the **Stellar Development Foundation (SDF)**. This was SDF's own liquidity position in the Blend pool — the single largest position by a wide margin.

| Field | Value |
|-------|-------|
| **Address** | `GCA34HBKNLWN3AOXWBRW5Y3HSGHCWF3UDBRJ5YHGU6HWGJZEPO2NSXI3` |
| **Type** | SDF-funded institutional multisig (11 signers) |
| **Funded by SDF** | ~511.8M XLM across 17 wires (98.8% of all inflows) |
| **Collateral at exploit** | 77.85M XLM — largest position in the pool |
| **Liquidated for** | ~70.68M XLM in fills #12–18 (2-minute window) |
| **Current holdings** | ~$11.5M: 11.1M USDC, 1.86M CETES, 919K XLM, 37.9K EURC |

The exploit was not just a DeFi hack — it was a **precision strike on an institutional liquidity position**, with two pre-positioned bots ([XLIQ](XLIQ.md) + [YHNF](YHNF.md)) ready to sweep the resulting auctions. SXI3 alone accounted for **93.6%** of all XLM seized across all 60 fills.

> **[→ Full SXI3 investigation including SDF funding chain, wire history, and shared signer proof](SXI3.md)**
